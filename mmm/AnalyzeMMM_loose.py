...
class AnalyzeMMM_loose(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    super(AnalyzeMMM_loose, self).__init__(tree, outfile, **kwargs)
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      if not bool(trigger50 or trigger24m3 or trigger24m1 or trigger24m2):
        continue
      ...
      if self.is_mc:
        ...
        if trigger24m1:
          tEff = self.triggerEff24(row.m1Pt, abs(row.m1Eta))
          weight = weight * tEff
        elif trigger24m2:
          tEff = self.triggerEff24(row.m2Pt, abs(row.m2Eta))
          weight = weight * tEff
        elif trigger24m3:
          tEff = self.triggerEff24(row.m3Pt, abs(row.m3Eta))
          weight = weight * tEff
        elif trigger50:

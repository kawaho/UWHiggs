...
for x in Lists.mc_samples:
    print x
    Lists.files.extend(glob.glob('../results/%s/AnalyzeEMValid2/%s' % (jobid, x)))
...
for j in Lists.procs:
    ...
    for h in Lists.bdthisto:
        plotter.plot_mc_vs_data('', ['VBF_LFV_HToEMu_M125*', 'GluGlu_LFV_HToEMu_M125*'], h[0], 100, xaxis = h[1], leftside=False, xrange=None, preprocess=None, show_ratio=False, ratio_range=1.5, sort=True, blind_region=False, control=s1, jets=j, year='2016', channel='emu')
        plotter.save(h[0]+j)

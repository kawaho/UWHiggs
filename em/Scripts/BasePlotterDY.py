...
class BasePlotterDY():
    ...
    def mcInit(self, files, lumifiles, outputdir, s):
        ...
        Wtotal = views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x : x.startswith('W1') or x.startswith('W2') or x.startswith('W3') or x.startswith('W4') or x.startswith('WGToLNuG') or x.startswith('WJetsToLNu'), Lists.mc_samples)])
        ...
        vbfHEM = views.StyleView(views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x : 'VBF_LFV_HToEMu' in x , Lists.mc_samples)]), **Lists.remove_name_entry(data_styles['VBF_LFV*']))
        ggHEM = views.StyleView(views.SumView( *[ plotter.get_view(regex) for regex in filter(lambda x : 'GluGlu_LFV_HToEMu' in x , Lists.mc_samples)]), **Lists.remove_name_entry(data_styles['GluGlu_LFV*']))

        plotter.views['vbfHEM']={'view' : vbfHEM }
        plotter.views['ggHEM']={'view' : ggHEM }

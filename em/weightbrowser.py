...
gROOT.SetBatch(True)
files = []
files.extend(glob.glob('results/Data2018_em/AnalyzeEMBDT/*root')) 
...
for f in files:
  ...
  fFile = TFile(f)
  f = f.replace('results/Data2018_em/AnalyzeEMBDT/','')
  fTreeS = fFile.Get("TreeB")
  nEntriesS = fTreeS.GetEntries()
  n = 0
  for i in range(nEntriesS):
    ...
    #n+=100./nEntriesS
  print f.replace('results/Data2018_em/AnalyzeEMBDT/',''), n
#print totalweight
print weights_prop

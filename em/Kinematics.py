...
var_d_star_gg = [
'mPt_Per_e_m_Mass','ePt_Per_e_m_Mass','emPt','emEta','j1Pt','DeltaEta_em_j1','DeltaPhi_em_j1','DeltaEta_em_j2','DeltaPhi_em_j2','m_met_mT','e_met_mT','DeltaPhi_e_met','DeltaPhi_m_met','DeltaEta_e_met','DeltaEta_m_met','MetEt','e_m_PZeta'
]
...
var_d_star_vbf = ['emPt','emEta','j1Pt','DeltaEta_em_j1','DeltaPhi_em_j1','DeltaEta_em_j2','DeltaPhi_em_j2','DeltaEta_j1_j2','DeltaPhi_j1_j2','Zeppenfeld','DeltaPhi_e_met','DeltaPhi_m_met','DeltaEta_e_met','DeltaEta_m_met','MetEt','R_pT']
...
def var_d_gg_0(myEle, myMuon, myMET, myJet1, myJet2, e_m_PZeta):
  return {'emEta' : (myEle + myMuon).Eta(), 'DeltaEta_m_met' : deltaPhi(myMuon.Phi(), myMET.Phi()), 'DeltaEta_e_met' : deltaPhi(myEle.Phi(), myMET.Phi()), 'MetEt' : myMET.Et(), 'DeltaPhi_e_met' : deltaPhi(myEle.Phi(), myMET.Phi()), 'emPt' : (myEle + myMuon).Pt(), 'DeltaPhi_em_j1' : -1, 'ePt_Per_e_m_Mass' :myEle.Pt()/visibleMass(myEle, myMuon) , 'e_met_mT' : transverseMass(myEle, myMET),'mPt_Per_e_m_Mass' :myMuon.Pt()/visibleMass(myEle, myMuon) ,'m_met_mT' :transverseMass(myMuon, myMET) ,'j1Pt' : -1,'DeltaEta_em_j1' : -1,'e_m_PZeta' :e_m_PZeta ,'DeltaEta_em_j2' : -1,'DeltaPhi_m_met': -1,'DeltaPhi_em_j2' : -1} 
...
def var_d_vbf(myEle, myMuon, myMET, myJet1, myJet2): 
  return {'emEta' : (myEle + myMuon).Eta(), 'R_pT': abs((myMuon+myEle+myJet1+myJet2).Pt())/(myMuon.Pt()+myEle.Pt()+myJet1.Pt()+myJet2.Pt()),'Zeppenfeld': Zeppenfeld(myEle, myMuon, myJet1, myJet2),  'DeltaPhi_j1_j2': deltaPhi(myJet1.Phi(), myJet2.Phi()), 'DeltaEta_j1_j2': deltaEta(myJet1.Eta(), myJet2.Eta()), 'DeltaEta_m_met': deltaPhi(myMuon.Phi(), myMET.Phi()), 'DeltaEta_e_met': deltaPhi(myEle.Phi(), myMET.Phi()), 'MetEt': myMET.Et(), 'DeltaPhi_e_met': deltaPhi(myEle.Phi(), myMET.Phi()), 'emPt': (myEle + myMuon).Pt(), 'DeltaPhi_em_j1': deltaPhi((myEle + myMuon).Phi(), myJet1.Phi()), 'j1Pt' : myJet1.Pt(), 'DeltaEta_em_j1': deltaEta((myEle + myMuon).Eta(), myJet1.Eta()), 'DeltaEta_em_j2': deltaEta((myEle + myMuon).Eta(), myJet2.Eta()), 'DeltaPhi_m_met': deltaPhi(myMuon.Phi(), myMET.Phi()), 'DeltaPhi_em_j2': deltaPhi((myEle + myMuon).Phi(), myJet2.Phi())} 

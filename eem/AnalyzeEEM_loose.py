...
class AnalyzeEEM_loose(MegaBase):
  ...
  def __init__(self, tree, outfile, **kwargs):
    ...
    self.muon50 = mcCorrections.muon50
    self.muon24 = mcCorrections.muonTrigger24	
    ...
    super(AnalyzeEEM_loose, self).__init__(tree, outfile, **kwargs)
  ...
  def process(self):
    ...
    for row in self.tree:
      ...
      #trigger24 = row.IsoMu24Pass and row.mMatchesIsoMu24Filter and row.mMatchesIsoMu24Path
      #trigger50 = row.Mu50Pass and row.mPt > 52
      if self.filters(row):
        continue

...
def make_puCorrector(dataset, kind=None, puname=''):
    ...
    if dataset in pu_distributions:
        return PileupWeight.PileupWeight( 'S6' if is7TeV else puname, 2017, *pu_distributions[dataset])
    else:
...
def make_puCorrectorUp(dataset, kind=None, puname=''):
    ...
    if dataset in pu_distributionsUp:
        return PileupWeight.PileupWeight( 'S6' if is7TeV else puname, 2017, *(pu_distributionsUp[dataset]))
    else:
        raise KeyError('dataset not present. Please check the spelling or add it to mcCorrectors.py')
...
def make_puCorrectorDown(dataset, kind=None, puname=''):
    ...
    if dataset in pu_distributionsDown:
        return PileupWeight.PileupWeight( 'S6' if is7TeV else puname, 2017, *(pu_distributionsDown[dataset]))
    else:
        raise KeyError('dataset not present. Please check the spelling or add it to mcCorrectors.py')
...
efficiency_trigger_mu_2017 = MuonPOGCorrections.make_muon_pog_IsoMu27_2017()
muonTracking = MuonPOGCorrections.mu_trackingEta_2017
eIDnoIsoWP80 = EGammaPOGCorrections.make_egamma_pog_electronID80noiso_2017()
...
Metcorected = RecoilCorrector("2017/Type1_PFMET_2017.root")

f_btag_eff = ROOT.TFile("../../../FinalStateAnalysis/TagAndProbe/data/2017/btag.root","r")
